      ...
      <div className="events-page">
        <div className="public-events">
          ...
          <div className="events-container">
            {events.filter(event => event.etype === 'Public').map(event => (
        ...
        <div className="personal-events">
          ...
          <div className="events-container">
            {events.filter(event => event.etype === 'Personal').map(event => (
              <div key={event._id} className="event-card">
                <img src={imageMap[event.photo]} alt={event.name} className="event-image" />
                <h3>{event.name}</h3>
                <p>Date: {event.date}</p>
                <p>Status:</p>
                <div className="status-box" style={{ backgroundColor: getStatusColor(event.estatus) }}>
                  <p>{event.estatus}</p>
                </div>
                {event.estatus === 'Declined' && <p style={{ color: 'red' }}>Reason: {event.reason}</p>}
              </div>
            ))}
          </div>
        </div>
